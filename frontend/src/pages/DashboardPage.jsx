...
const DashboardPage = () => {
  ...
  // --- Modern API fetching using async/await + Promise.all
  const fetchDashboardData = async () => {
    try {
      ...
      const [statsRes, challengesRes, activityRes] = await Promise.all([
  ...
  // --- Fetch data on mount and poll every 60 seconds
  useEffect(() => {
    fetchDashboardData();
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        />
        <StatCard
          icon={<FaTrophy className="h-6 w-6 text-secondary-600" />}
          label="Completed Challenges"
          value={stats?.completedChallenges}
          color="secondary"
        />
        <StatCard
          icon={<FaRunning className="h-6 w-6 text-success-600" />}
          label="Active Challenges"
          value={stats?.activeChallenges}
          color="success"
        />
        <StatCard
          icon={<FaGift className="h-6 w-6 text-warning-600" />}
      ...
      {/* Active Challenges & Recent Activity (placeholder for now) */}
      {/* You can later map activeChallenges and recentActivity here */}
    </div>
  );
};
...
const StatCard = ({ icon, label, value, color }) => (
  <div className="bg-white rounded-lg shadow p-6 flex items-center">
    <div className={`rounded-full p-3 mr-4 bg-${color}-100`}>{icon}</div>
    <div>
